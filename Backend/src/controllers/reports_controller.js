...
const eliminarReporte = async (req, res) => {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) {
        return res.status(404).json({ msg: "Proporcionar un id válido" });
    }

    const reporte = await Reports.findByIdAndDelete(id);
    if (!reporte) {
        return res.status(404).json({ msg: "Reporte no encontrado" });
    }

    res.status(200).json({ msg: "Reporte eliminado con éxito" });
};
